function searchServiceBrand(name, token) {
    $(".loader").fadeIn();
    $.ajax(
        {
            type:'POST',
            url:'/manage/serviceBrand/search',
            data:{
                '_token' : token,
                'name' : name,
            },
...
function startUpdateServiceBrand(id, token, brandId, nameId, picId) {
    $(".loader").fadeIn();
    $.ajax(
        {
            type:'POST',
            url:'/manage/serviceBrand/startUpdate',
            data:{
                '_token' : token,
                'id' : id,
            },
...
function updateServiceBrand(status, brandId, name, picId, token) {
    $(".loader").fadeIn();
    if(status){
        if($(name).val() === ""){
            ...
            $.ajax(
                {
                    ...
                    url:'/manage/serviceBrand/Update',
                    data:fd,
                    processData:false,
                    contentType:false,
                    success:function (data) {
                        ...
                        window.location.href = '/manage/serviceBrand';
...
function deleteServiceBrand(id, token) {
    ...
    startDelete.fire({
        ...
        if (result.value){
            ...
            $.ajax(
                {
                    type:'POST',
                    url:'/manage/serviceBrand/delete',
                    data:{
                        '_token' : token,
                        'id' : id,
                    },
